...
#[string]$OutputPath = "C:\ProgramData\CentraStage\Logs\Thor"
...
try {
    ...
    Write-Log "Adding random delay to the scan start (max. $($RandomDelay)): sleeping for $($LocalDelay) seconds" 
    ...
    try {
        ...
        # Asgard Instance
        if ( $AsgardServer -ne "" ) {
            Write-Log "Attempting to download THOR from $AsgardServer" 
        ...
        elseif ( $UseThorCloud ) {
            Write-Log 'Attempting to download THOR from Nextron cloud portal, please wait ...' 
